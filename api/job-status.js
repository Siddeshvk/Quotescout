...
// Deterministic rules engine (lib/rules.js). Runs over the OCC/step geometry that
// analyze.js persisted in job.rules_context, then merges source-tagged flags into
// the AI's flags. Fully guarded — absent context or any error falls back to AI flags.
import { evaluateRules, mergeFlags } from '../lib/rules.js';
...
async function processJob(jobId) {
  ...
  if (batch.processing_status === 'ended') {
    ...
    // 8. Deterministic rules post-pass + merge.
    // Run lib/rules.js over the geometry analyze.js persisted at submit time
    // (job.rules_context: OCC features preferred, step-probe signals as fallback)
    // plus the AI's dimensions, then merge the source-tagged deterministic flags
    // into the AI flags. Guarded: missing column/data or any error => AI flags alone.
    const aiFlags = Array.isArray(analysisResult.flags) ? analysisResult.flags : [];
    let ruleFlags = [];
    try {
      const rc = job.rules_context;
      if (rc && Array.isArray(rc.geometries) && rc.geometries.length) {
        const dims = Array.isArray(analysisResult.dimensions) ? analysisResult.dimensions : [];
        for (const geo of rc.geometries) {
          const r = evaluateRules({
            geometry: geo.occ || geo.step_signals || null,
            dimensions: dims,
            profile: rc.profile_text || null,
          });
          if (r && Array.isArray(r.flags)) ruleFlags = ruleFlags.concat(r.flags);
        }
      }
    } catch (e) {
      console.error(`Rules post-pass failed for ${jobId} (non-fatal):`, e?.message);
      ruleFlags = [];
    }
    const flags = ruleFlags.length ? mergeFlags(aiFlags, ruleFlags) : aiFlags;
    if (ruleFlags.length) {
      console.log(`Job ${jobId}: merged ${ruleFlags.length} deterministic flag(s) with ${aiFlags.length} AI flag(s) -> ${flags.length} total.`);
    }
