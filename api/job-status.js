      ...
      for await (const item of stream) {
        ...
        if (item.result.type === 'succeeded') {
          ...
          try {
            ...
            // Try to extract outermost JSON
            const firstBrace = aiText.indexOf('{');
            const lastBrace = aiText.lastIndexOf('}');
            if (firstBrace !== -1 && lastBrace > firstBrace) {
              try {
                analysisResult = JSON.parse(aiText.slice(firstBrace, lastBrace + 1));
              } catch (e2) {
                resultError = `JSON parse failed: ${e2.message?.slice(0, 100)}. AI response started: "${aiText.slice(0, 150)}"`;
              }
            } else {
              resultError = `AI response was not JSON. First 200 chars: "${aiText.slice(0, 200)}"`;
            }
          }
