...
export default async function handler(req, res) {
  ...
  try {
    ...
    const wantsEmail = (fields.wants_email?.[0] || '').toString().toLowerCase() === 'true';
    ...
    const shopProfileRaw = (fields.shop_profile?.[0] || '').trim();
    ...
    const ocrTexts = fields.ocr
      ? (Array.isArray(fields.ocr) ? fields.ocr : [fields.ocr])
      : [];
    ...
    const fileNames = rawFiles.map(f => f.originalFilename || 'unknown').join(' + ');
    ...
    try {
      ...
      // Defensive: if the migration hasn't run yet (no ip_address column), retry without
      // it so job recording never breaks on a deploy-order mistake.
